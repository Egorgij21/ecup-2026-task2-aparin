...
from src.prompts import RULES_BAD, clean  # noqa: E402
...
ГЛАВНОЕ: внимательно ПРОЧИТАЙ ТЕКСТ НА УПАКОВКЕ на фотографиях. Официальное правило
площадки гласит, что товар является БАД, если прямое указание на это есть В ОПИСАНИИ
ИЛИ НА ИЗОБРАЖЕНИИ. Поэтому маркировка на упаковке решает.

Признаки того, что товар — БАД (ответ «Да»):
* надписи «БАД», «биологически активная добавка», «добавка к пище»,
  «dietary supplement», «food supplement», «пищевая добавка»;
* «не является лекарственным средством» — эта фраза обязательна для БАД;
* номер свидетельства о государственной регистрации (СГР), «СоГР», «RU.77.99»;
* «рекомендуется как дополнительный источник», «БАД к пище».

Признаки того, что это НЕ БАД (ответ «Нет»):
* маркировка спортивного питания: «sport nutrition», протеин, whey, гейнер,
  BCAA, креатин, аминокислоты, предтренировочный комплекс, «для набора массы»;
* это лекарство: «лекарственное средство», «инструкция по применению препарата»,
  рецептурный отпуск;
* косметика, продукт питания, напиток, бытовой товар без маркировки добавки;
* явная надпись «не является БАД».

Если надписей на фотографиях не разобрать, опирайся на текст карточки."""

QUESTION = "Является ли этот товар биологически активной добавкой (БАД)?"
...
def main():
    ...
    d = df[df["category"] == "БАД"].reset_index(drop=True)
    d["n"] = d["name"].fillna("").str.lower().str.replace("ё", "е")
    d["grp"] = ""
    # группировка не нужна: считаем всю категорию целиком
    d["grp"] = "все"
    sel = d.copy()
    sel["paths"] = [imgs_for(p) for p in sel["id"]]
    sel = sel[sel["paths"].map(len) > 0].reset_index(drop=True)
    print(f"вся БАД-категория: {len(sel)} строк, позитивов {int(sel.label.sum())}", flush=True)
    ...
    def run(mode):
        ...
        for i in range(0, len(sel), BS):
            ...
            for _, r in ch.iterrows():
                ...
                head = TARGETED if mode == "targeted" else RULES_BAD
                q = QUESTION if mode == "targeted" else \
                    "Является ли этот товар биологически активной добавкой по правилам выше?"
    ...
    sel["targeted"] = run("targeted")
    sel[["id", "category", "label", "targeted"]].to_parquet(ROOT + "exp/vlm_bad_scores.parquet")
    from sklearn.metrics import average_precision_score
    y = sel["label"].values
    print(f"\nВСЯ БАД: n={len(y)} поз={int(y.sum())} "
          f"AUC={roc_auc_score(y, sel['targeted'].values):.4f} "
          f"PR={average_precision_score(y, sel['targeted'].values):.4f} "
          f"-> exp/vlm_bad_scores.parquet", flush=True)
    raise SystemExit
