...
def main():
    ...
    print(f"вся флам-категория: {len(sel)} строк, позитивов {int(sel.label.sum())}", flush=True)
    ...
    sel[["id", "category", "label", "targeted"]].to_parquet(ROOT + "exp/vlm_flam_scores.parquet")
    from sklearn.metrics import average_precision_score
    y = sel["label"].values
    print(f"\nВСЯ флам: n={len(y)} поз={int(y.sum())} "
          f"AUC={roc_auc_score(y, sel['targeted'].values):.4f} "
          f"PR={average_precision_score(y, sel['targeted'].values):.4f} "
          f"-> exp/vlm_flam_scores.parquet", flush=True)
    raise SystemExit
