...
from src.prompts import (build_messages, build_messages_badimg,  # noqa: E402
                         build_messages_flamtypes, build_messages_noexcl,
                         build_messages_sportbad)
from src.rules import extract                          # noqa: E402

HERE = Path(__file__).resolve().parent
ART = HERE / "artifacts"
ADAPTERS = [HERE / "lora_adapter", HERE / "lora_adapter2"]
# v21: ТРЕТИЙ флам-адаптер, обученный с прицельным списком типов (_FLAM_TYPES).
# Декоррелирован с базовыми (OOF corr 0.47–0.63 против 0.5 между базами) — бленд трёх
# даёт флам-F1 0.8714→0.8786 на OOF, но НИЖЕ шумового пола ±0.063 (диверсификация,
# не подтверждённый прирост). Скорится СВОИМ промптом (build_messages_flamtypes).
ADAPTER_FLAMTYPES = HERE / "lora_adapter3"
...
# v23: БАД тоже 3-бленд промптов (как v22). Диверсификация обеих категорий.
...
def preflight():
    ...
    for f in ("adapter_config.json", "adapter_model.safetensors"):
        if not ADAPTER_FLAMTYPES.joinpath(f).exists():
            raise FileNotFoundError(f"нет обязательного файла: {ADAPTER_FLAMTYPES / f}")
    ft_base = adapter_base_subpath(str(ADAPTER_FLAMTYPES))
    resolve_model(ft_base)
    print(f"[preflight] флам-адаптер flamtypes: база {ft_base}", flush=True)
    ...
    return bases, bad_base, ft_base


def main():
    t0 = time.time()
    args = parse_args()
    bases, bad_base, ft_base = preflight()
    ...
    # v21: третий флам-адаптер обучался с прицельным списком типов — на прогоне
    # обязан получать ТОТ ЖЕ промпт (build_messages_flamtypes), иначе train/inference
    # рассинхрон. Только флам; БАД/ансамбль/два базовых флам-адаптера не трогает.
    lora_scores.append(score_cards(ft_base, cats, names, descs, build_messages_flamtypes,
                                    flam_idx, adapter_dir=str(ADAPTER_FLAMTYPES)))
    ...
    # v23: 3-бленд БАД — три адаптера, КАЖДЫЙ со своим обучающим промптом.
    bad_parts = []
